...
public class OID4VPEndpoint {
    ...
    public Response getRequestObject(@PathParam("request_handle") String requestHandle) {
        ...
        OID4VPAuthorizationRequest authorizationRequest = provider.createAuthorizationRequest(responseUri, handleReference.getState(), responseUri);
    ...
    public Response directPost(@BeanParam OID4VPDirectPostRequest directPostRequest) {
        ...
        OID4VPIdentityProvider.OID4VPRequestHandleReference stateReference = readStored(
        ...
        int lifespan = Math.max(60, provider.getConfig().getRequestObjectLifespan());
        session.singleUseObjects().put(
                OID4VPIdentityProvider.storageKey(OID4VPIdentityProvider.DIRECT_POST_RESULT_PREFIX, responseCode),
                lifespan,
                Map.of(OID4VPIdentityProvider.ENTRY_JSON, JsonSerialization.valueAsString(directPostRequest)));
    ...
    public Response continueAuthentication(@QueryParam(OID4VPConstants.STATE) String state,
                                           @QueryParam("response_code") String responseCode) {
        ...
        OID4VPDirectPostRequest result = removeStored(
                OID4VPIdentityProvider.storageKey(OID4VPIdentityProvider.DIRECT_POST_RESULT_PREFIX, responseCode),
                OID4VPDirectPostRequest.class);
        if (result == null) {
            return callback.error(provider.getConfig(), "Missing direct_post result");
        }

        if (result.isErrorResponse()) {
            String message = Optional.ofNullable(result.getErrorDescription()).orElse(result.getError());
        ...
        BrokeredIdentityContext identity = new BrokeredIdentityContext(OID4VPIdentityProvider.STATIC_SUBJECT, provider.getConfig());
