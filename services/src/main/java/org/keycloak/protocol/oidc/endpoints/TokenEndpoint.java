...
=======
    ...
    public Response codeToToken() {
        checkAndRetrieveDPoPProof(Profile.isFeatureEnabled(Profile.Feature.DPOP) && !clientConfig.isUseDPoPForRefreshTokensOnly());
    ...
    private void checkAndBindDPoPToken(TokenManager.AccessTokenResponseBuilder responseBuilder, boolean useRefreshToken, boolean isDPoPSupported) {
        ...
        if (clientConfig.isUseDPoP() || dPoP != null) {
            // Only Refresh Tokens might be DPoP-bound, as described in https://datatracker.ietf.org/doc/html/rfc9449#section-5-9
            if (!clientConfig.isUseDPoPForRefreshTokensOnly()) {
