...
Additionally, a `none` cache mode is available. When `--cache=none` is set, {project_name} will completely disable Infinispan caching. This mode can be useful for specific development scenarios, for running in highly resource-constrained environments where caching is not desired, or during certain types of troubleshooting and testing. When this mode is active, {project_name} will attempt to load a minimal configuration from `conf/cache-none.xml` which defines no caches.
