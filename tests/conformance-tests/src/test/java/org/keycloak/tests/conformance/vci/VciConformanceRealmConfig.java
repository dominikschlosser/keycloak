...
public class VciConformanceRealmConfig implements RealmConfig {
    ...
    private static final JsonNode ATTESTER_JWKS = loadAttesterJwks();

    public static JsonNode attesterJwks() {
        return ATTESTER_JWKS;
    }

    private static JsonNode loadAttesterJwks() {
        try (InputStream input = VciConformanceRealmConfig.class.getResourceAsStream("/conformance/vci-attester-jwks.json")) {
            if (input == null) {
                throw new IllegalStateException("Missing VCI conformance attester JWKS");
            }
            return JsonSerialization.readValue(input, JsonNode.class);
        } catch (IOException e) {
            throw new RuntimeException("Failed to load VCI conformance attester JWKS", e);
        }
    }
    ...
    private IdentityProviderRepresentation attesterTrustIdentityProvider() {
        ...
        trust.setConfig(Map.of(DefaultTrustIdentityProviderConfig.TRUSTED_JWKS, ATTESTER_JWKS.toString()));
        return trust;
    }
